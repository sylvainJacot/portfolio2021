...
export const MailToStyleWrapper = styled.a`
  position: relative;
  display: flex;
  align-items: center;
...
const MailTo = ({email,subject,body,label,src}) => {
    return <>
        <MailToStyleWrapper href={`mailto:${email}?subject=${encodeURIComponent(subject) || ''}&body=${encodeURIComponent(body) || ''}`}>
